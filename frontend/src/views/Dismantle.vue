<template>
  <div class="page">
    <div class="card">
      ...
      <!-- Step 1: 素材录入 -->
      <div class="section-title">Step 1: 录入原始素材</div>
      <el-form :model="materialForm" label-width="100px">
        ...
        <el-form-item label="平台">
          <el-select v-model="materialForm.platform" placeholder="选择平台">
        ...
        <el-form-item label="品类">
          <el-select v-model="materialForm.category" placeholder="选择品类">
    ...
    <div class="card" v-if="currentMaterial">
      ...
      <el-form :model="dismantleForm" label-width="100px">
        ...
        <!-- L2 策略层 -->
        <div class="layer-header l2">L2 策略层</div>
        <el-form-item label="策略标签">
          <el-select v-model="dismantleForm.l2_strategy" multiple placeholder="选择策略">
        ...
        <!-- L3 结构层 -->
        <div class="layer-header l3">L3 结构层</div>
        <div v-for="(section, idx) in dismantleForm.l3_structure" :key="idx" class="structure-row">
          ...
          <el-input v-model="section.function" placeholder="功能" style="width:180px" />
          <el-input-number v-model="section.ratio" :min="1" :max="100" style="width:100px" />
          <span style="margin-left:8px;color:#999">%</span>
          <el-button type="danger" link @click="removeSection(idx)">删除</el-button>
        </div>
        <el-button type="primary" link @click="addSection" style="margin-left:100px">+ 添加段落</el-button>

        <!-- L4 元素层 -->
        <div class="layer-header l4">L4 元素层</div>
        ...
        <!-- L5 表达层 -->
        <div class="layer-header l5">L5 表达层</div>
        <el-form-item label="金句">
          <el-select v-model="dismantleForm.l5_expressions.golden_sentences" multiple allow-create filterable placeholder="输入金句后回车">
          </el-select>
        </el-form-item>
        <el-form-item label="数据">
          <el-select v-model="dismantleForm.l5_expressions.data_refs" multiple allow-create filterable placeholder="输入数据后回车">
          </el-select>
        </el-form-item>
        <el-form-item label="视觉">
          <el-select v-model="dismantleForm.l5_expressions.visual_desc" multiple allow-create filterable placeholder="输入视觉描述后回车">
          </el-select>
        </el-form-item>

        <el-form-item>
          <el-button type="primary" @click="submitDismantle" :loading="loading">
            保存拆解结果
          </el-button>
          <el-button type="success" @click="submitDismantleAndExtract" :loading="loading">
            保存并提取骨架
          </el-button>
        </el-form-item>
      </el-form>
...
.section-title { font-size: 14px; font-weight: 600; color: #666; margin: 16px 0; padding-left: 12px; border-left: 3px solid #667eea; }
.layer-header { font-size: 13px; font-weight: 700; color: #fff; padding: 6px 16px; border-radius: 6px; margin: 16px 0 8px; }
...
.structure-row { display: flex; align-items: center; gap: 8px; margin-bottom: 8px; }
</style>
